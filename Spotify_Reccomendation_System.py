...
from spotify_recs_helpers import get_distance_recs_playlist_1
from spotify_recs_helpers import get_distance_recs_playlist_2
from spotify_recs_helpers import get_distance_recs_playlist_gower
from spotify_recs_helpers import create_playlist
import json
...
def main():
    # get Spotify Application credentials that allows us to access the Spotify API
    ...
    # read data that has language flag
    data = pd.read_csv('spoty_new_w_langflag.csv').drop(columns =['Unnamed: 0','level_0'])

    # drop songs that are childrens songs
    data = data[data.track_genre!='children'].reset_index()

    # create data that only has numerical features
    data_num = data[num_features]

    # create standard scaler and fit it to data
    ...
    # if the user wants song titles to be mostly in english, drop songs that do not have titles in English
    while True: 
        english = input("Would you like most song titles to be in English? (Y/N) ")
    ...
    # fit the data using the previously created scaler
    final_numerical = pd.DataFrame(scale_fit.transform(final_numerical),columns=num_features)
    
    # check if the user wants to user Gower distance (include Genre) or Euclidean
    while True: 
